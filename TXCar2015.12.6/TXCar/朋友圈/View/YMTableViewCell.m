...
#pragma mark -  //头像 昵称 简介
    ...
    _userHeaderImage.image = [UIImage imageNamed:tempDate.messageBody.posterImgstr];
      NSString * iiiiiii=tempDate.messageBody.posterImgstr;
    NSURL *URL = [NSURL URLWithString:iiiiiii];
    NSData* data = [NSData dataWithContentsOfURL:URL];
    _userHeaderImage.image=[UIImage imageWithData:data];
...
#pragma mark - /////// //图片部分
    ...
    for (int  i = 0; i < [ymData.showImageArray count]; i++) {
        ...
        [image sd_setImageWithURL:[ymData.showImageArray objectAtIndex:i]];
...
//        [self.contentView addSubview:_ilcoreText];
    ...
    if (ymData.replyDataSource.count == 0) {//没回复的时候
       ...
       _replyBtn.frame = CGRectMake(screenWidth - offSet_X - 40 + 6,TableHeader + 30 + ShowImage_H + (ShowImage_H + 10)*(scale_Y/3) + origin_Y + hhhh + kDistance  + balanceHeight + kReplyBtnDistance - 24-20, 40, 18);
